...
struct CoffFile {
    ...
    Slice<ExternalJump> jumptable;
};

static Slice<CoffSymbol> coff_symbol_table(CoffFile *file) {
    IMAGE_FILE_HEADER *coff_header = (IMAGE_FILE_HEADER *)file->base;
    CoffSymbol *symbol_base = (CoffSymbol *)(file->base + coff_header->PointerToSymbolTable);
...
static bool coff_load_file(CoffFile *file, const char *path) {
    file->base = read_file("tests/test.obj");
    if (!file->base) {
    ...
    section_headers.data = (IMAGE_SECTION_HEADER *)(file->base + sizeof(IMAGE_FILE_HEADER));
    ...
    for (IMAGE_SECTION_HEADER &section : section_headers) {
        ...
        if (loadable && !discardable) {
            memcpy(cursor, file->base + section.PointerToRawData, section.SizeOfRawData);
    ...
    for (IMAGE_SECTION_HEADER &section : section_headers) {
        ...
        if (section_runtime_base) {
            CoffRelocation *relocations_base = (CoffRelocation *)(file->base + section.PointerToRelocations);
